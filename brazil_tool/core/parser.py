...
def first_company_like_after(header_block: str, full_text: str) -> Optional[str]:
    ...
    if idx < 0:
        m = re.search(r'DESTINAT[ÁA]RIO/REMETENTE|DESTINATARIO/REMETENTE', full_text, flags)
...
def parse_invoice_from_text(text: str, file_name: str) -> Invoice:
    ...
    inv.numero = get_after_label(r'Nro\.\s*(?:Nota\s*)?:?', r'(\d{1,3}(?:\.\d{3}){0,2}|\d{1,9})', text, 400)
    ...
    if dest_block:
        dest_cnpj_cpf = get_after_label(r'CNPJ/CPF', r'(' + CNPJ_RE + r'|' + CPF_RE + r')', dest_block, 120)
        if dest_cnpj_cpf:
        ...
        if not inv.destinatario_nome:
            ...
            if "Nome/Razao Social" in dest_block:
                lines = [l.strip() for l in dest_block.splitlines() if l.strip()]
                # Find the index of the label
                try:
                    idx_label = -1
                    for i, ln in enumerate(lines):
                        if "Nome/Razao Social" in ln:
                            idx_label = i
                            break
                    
                    if idx_label >= 0:
                        # The value is usually at a fixed offset after the last label 
                        # Or we look for the first line that isn't a label and isn't too short
                        labels = {"CNPJ/CPF", "Data da Emissao", "Endereco", "Bairro/Distrito", "CEP", 
                                  "Data de Saida", "Municipio", "Fone/Fax", "UF", "Inscricao Estadual", "Hora"}
                        for ln in lines[idx_label+1:]:
                            if len(ln) > 5 and not any(lab in ln for l in labels for lab in lab.split()):
                                if not re.search(r'\d{2}/\d{2}/\d{4}', ln): # Skip dates
        ...
        inv.destinatario_fone = get_after_label(r'(?:Telefone|Fone/?Fax?)', r'(' + PHONE_RE + r')', dest_block, 160)
    ...
    calc_block = extract_block(text,
        r'(?=Base\s+de\s+Calculo\s+ICMS|Base\s+de\s+C[áa]lculo\s+do\s+ICMS)',
    ...
    if calc_block:
        ...
        inv.base_calculo_icms = grab_in_calc(r'(?:Base\s+de\s+C[áa]lculo\s+(?:do\s+)?ICMS)')
        inv.total_icms = grab_in_calc(r'(?:Valor\s+do\s+ICMS)')
        inv.base_calculo_st = grab_in_calc(r'(?:Base\s+de\s+C[áa]lculo\s+de\s+Subst\.?\s+Trib\.?|Base\s+de\s+C[áa]lc\.?\s+do\s+ICMS\s+Subst\.?)')
        inv.valor_icms_st = grab_in_calc(r'(?:Valor\s+do\s+ICMS\s+Subst\.?\s+Trib\.?|Valor\s+do\s+ICMS\s+Substitui[cç][aã]o)')
        inv.total_nota = grab_in_calc(r'(?:Valor\s+total\s+da\s+Nota|Valor\s+Total\s+da\s+Nota)')
        inv.total_ipi  = grab_in_calc(r'(?:Valor\s+total\s+do\s+IPI|Valor\s+do\s+IPI)')
        inv.frete      = grab_in_calc(r'(?:Valor\s+do\s+frete|Frete\b)')
        inv.seguro     = grab_in_calc(r'(?:Valor\s+do\s+seguro|Seguro\b)')
        inv.desconto_total = grab_in_calc(r'(?:Desconto\b)')
        inv.outras_despesas = grab_in_calc(r'(?:Outras\s+despesas\s+acess[oó]rias|Outras\s+Despesas)')
    ...
    block = extract_block(text, r'DADOS\s+DO(S)?\s+PRODUTO(S)?/SERVI[CÇ]O(S)?',
        r'INFORMA[cç][OÕ]ES\s+COMPLEMENTARES|DADOS\s+ADICIONAIS|Reservado\s+ao\s+Fisco|CALCULO\s+DO\s+IMPOSTO|C[ÁA]LCULO\s+DO\s+IMPOSTO') or text
    ...
    for entry in raw_entries:
        ...
        if len(map_values) >= 7 and re.search(r"(\d+[\,\.]\d{2})\s+(\d+[\,\.]\d{2})\s*$", entry):
            ...
            for m in re.finditer(r"(\d+[\,\.]\d{2})\s+(\d+[\,\.]\d{2})(?=\s+[A-Za-z])", entry):
                aliq_pair_match = m
        ...
        if m_quant:
            ...
            if len(map_values) >= 2:
                it.valor_unitario = br_to_float(map_values[0])  # Vl. Unit. 
        ...
        if aliq_end_index > 0 and aliq_end_index < len(entry):
            ...
            if all_money_positions:
                ...
                clean_tail = re.sub(r'\b\d{8}\b', '', raw_tail)  # NCM
                clean_tail = re.sub(r'\b\d{3}\b', '', clean_tail)  # CST
                clean_tail = re.sub(r'\b\d{4}\b', '', clean_tail)  # CFOP
